...
I'm with Gosling here.  [Google totally slimed Sun.](http://nighthacks.com/jag/blog/393/index.html)  It's impossible to express just how justified "slimed" feels.  Java wasn't Java because of `max`, `sort`, and a hundred more method signatures like them.  That's like saying _Dark Side of the Moon_ is just a bunch of well tuned guitars playing chords.  But `max` and `sort` might have been all the Supremes were really given to see.

## Inextricable, Extricated

Given how much of the total impression of Java stems from its holism and consistency, it's particularly tough to see both majority and dissent repeating the idea that Java's declaring code, or API, was "inextricable" from its implementing code.  For one, Google apparently extricated it.  For two, that's the point of an API toward the symbolic end of the machine-symbol spectrum.
