...
<!-- Overall, this approach is distinguishable from Rack's approach in Ruby, which makes request information available on a single `Hash` object argument and expects an `Array` with a status code, a `Hash` of headers, and a body object that responds to `.each` with successive `String` values. The return value is the Rack sever's response, rather than the state of the `http.ServerResonse` object when the `createServer` function returns. Chaining is accomplished by initializing middleware objects with a pointer to the next middleware object in the desired stack. Middleware themselves are responsible for passing an environment hash on to the next middleware. -->
